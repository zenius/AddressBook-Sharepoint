		...
		for (var d = h.length - 1; d >= 0; d--) {
			...
			if (typeof b !== 'number')
				throw Error.argument(
					'value',
					String.format(Sys.Res.enumInvalidValue, c.split(',')[d].trim(), this.__typeName)
				);
			j |= b;
		}
		return j;
...
Sys.Observer.clear = function(a) {
	...
	Sys.Observer._collectionChange(
		a,
		new Sys.CollectionChange(Sys.NotifyCollectionChangedAction.reset, null, -1, b, 0)
	);
};
...
Sys.Observer.remove = function(a, b) {
	...
	if (c !== -1) {
		...
		Sys.Observer._collectionChange(
			a,
			new Sys.CollectionChange(Sys.NotifyCollectionChangedAction.remove, null, -1, [b], c)
		);
		return true;
	}
	return false;
...
Sys.Observer.removeAt = function(b, a) {
	if (a > -1 && a < b.length) {
		...
		Sys.Observer._collectionChange(
			b,
			new Sys.CollectionChange(Sys.NotifyCollectionChangedAction.remove, null, -1, [c], a)
		);
	}
};
	...
	for (; true; ) {
		...
		switch (i[0]) {
			...
			case 'zzz':
				c = this.getTimezoneOffset() / 60;
				a.append(
					(c <= 0 ? '+' : '-') + d(Math.floor(Math.abs(c))) + ':' + d(Math.abs(this.getTimezoneOffset() % 60))
				);
				break;
			case 'g':
			case 'gg':
...
Sys.Net.XMLHttpExecutor.prototype = {
	...
	get_xml: function() {
		...
		if (
			a.documentElement.namespaceURI === 'http://www.mozilla.org/newlayout/xml/parsererror.xml' &&
